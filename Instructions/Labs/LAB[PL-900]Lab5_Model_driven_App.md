...
> [!NOTE]
> 有時候，當使用設計工具時，系統會提示您登入。 選取要取消登入畫面的 X。

## 工作 2：編輯模型驅動應用程式以符合您的需求

現在已建立應用程式，我們將對應用程式呈現的方式進行一些變更。 我們要有兩個不同的群組：人員和事件。 我們想要在 [人員] 群組中擁有 [聯繫人] 數據表，以及 [事件] 群組中的所有 [事件] 數據表。
...
## 工作 3：編輯模型驅動應用程式中的不同表單和檢視，以符合您的需求。
...
15. **在**左側流覽下方，將滑鼠停留在 [聯繫人] 檢視**上**，然後選取 [**編輯**] 圖示。
16. 選取 **[+ 檢視數據行**]。
17. 搜尋並將 [ **聯繫人類型]** 資料行新增至表單。
18. 選取 [ **儲存併發佈]** 按鈕。
...
請注意，可能有 **EventDetails1 數據** 行。 如果是，我們會從檢視中移除它。 *（如果你沒有它，* **您可以跳至步驟 28***）*

20.  將滑鼠停留在 [ **事件] 檢視** 上，然後選取 [ **編輯]** 按鈕。

如果系統提示您儲存**** 變更，請選取 [**儲存並繼續**]。

21.  在 [**檢視]** 上，選取 EventDetails1** 旁**的箭號，然後從出現的功能表中選取 [**移除**]。

![顯示如何從檢視中移除額外數據行的螢幕快照。](media/f0f1484a4f3a679abc18cdb1cd5c04da.png)

22.  選取 [ **儲存併發佈]** 按鈕。
23.  選取 [ **上一步] 箭頭** 按鈕以返回應用程式設計工具。
24.  在 [導覽] 底**下，將滑鼠停留在 **[事件****] 窗體**上，然後選取 [**編輯**]。**

    如果系統提示您儲存變更，請選取 [ **儲存並繼續**]。

25.  選取 **[事件詳細數據1]** 字段，然後按 **鍵盤上的 [刪除** ] 鍵。
...
26.  選取 [ **儲存併發佈]** 按鈕。
27.  選取 [ **上一步] 箭頭** 按鈕以返回應用程式設計工具。

如果您窗體上沒有 EventDetail1 欄位，請在這裡繼續。

28.  在 **應用程式的命令行** 上，選取 [ **儲存併發佈]** 按鈕。
29.  選取 [ **上一頁** ] 按鈕以返回 **Power Apps 製作者** 入口網站。
30.  選取 **[回到解決方案** ] 以返回主要 **Power Apps** 畫面。
...
3.  **將您剛才建立的 Course 事件管理**應用程式暫留，然後選取 **[播放**] 圖示。
4.  使用左側導覽，選取 [ **聯繫人**]。
5.  在命令行上，選取 [ **+ 新增** ] 按鈕。
6.  在 [ **新增聯繫人]** 畫面中，設定如下：
    -   **名字：** 蘇珊娜
...
![已完成工作階段註冊表單的螢幕快照。  ](media/16cccd9ebb949ba0f364211f1ee55d4d.png)
