...
6. 按一下 [儲存]。 此動作會開始在背景佈建資料表，同時您也可以開始新增其他資料行。

7. 建立資料表之後，在 [結構描述] 區段下方選取 [資料行]。 
...
8. 找到並選取您稍早已下載的 **Visits.xls** 檔案。 (請注意，上傳檔案可能會需要一或兩分鐘的時間。 如果出現對應錯誤的訊息，請無需擔心，我們稍後會修正這些問題。)
