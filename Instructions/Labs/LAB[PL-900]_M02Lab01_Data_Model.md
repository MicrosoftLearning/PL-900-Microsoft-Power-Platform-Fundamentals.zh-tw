...
# <a name="exercise-1-create-solution"></a>練習 \#1：建立解決方案

## <a name="task-1-create-solution-and-publisher"></a>工作 \#1：建立解決方案和發佈者

1.  建立方案

    -   瀏覽至 <https://make.powerapps.com>。 您可能需要重新驗證：如有需要，請按一下 **[登入]** ，然後依照指示操作。
...
3.  完成解決方案建立作業。
    ...
    -   接著按一下 [發行者] 下拉式清單，然後選取您剛才建立的發行者 **Bellows College**。

    -   驗證 **版本** 是否設為 **1.0.0.0** 
    
    -   按一下 [建立]。

# <a name="exercise-2-add-existing-and-create-new-tables"></a>練習 \#2：加入現有的資料表及建立新的資料表

**目標：** 在此練習中，您將新增標準「連絡人」資料表，並針對解決方案中的「大樓」和「造訪情形」建立新的自訂資料表。 

## <a name="task-1-add-existing-table"></a>工作 \#1：新增現有資料表
...
6.  選取 **[檢視]** 索引標籤，然後選取 **[現行連絡人]** 檢視。 按一下 [新增]。
...
## <a name="task-2-create-building-table"></a>工作 2：建立「大樓」資料表

1.  您仍應針對 [校園管理] 解決方案開啟瀏覽器。 如果沒有，請依照下列步驟開啟 [校園管理] 解決方案：

    * 登入 <https://make.powerapps.com> (若您還未登入)。
    
    * 選取 [解決方案]，然後按一下以開啟您剛才建立的 **[(您的姓氏) 校園管理]** 解決方案。
...
## <a name="task-3-create-visit-table-and-columns"></a>工作 #3：建立「造訪情形」資料表和資料行
...
# <a name="exercise-3-create-relationships"></a>練習 \#3：建立關聯性

**目標：** 在此練習中，您將新增資料表之間的關聯性。

## <a name="task-1-create-relationships"></a>工作 #1：建立關聯性
...
# <a name="exercise-4-import-data"></a>練習 \#4：匯入資料

**目標：** 在此練習中，您會將範例資料匯入至 Dataverse 資料庫。

## <a name="task-1-import-solution"></a>工作 #1：匯入解決方案
...
>
>   此訊息表示資料模型不完整、發行者的前置詞不是 **bc**，或 **Building** 和 **Visit** 資料表的名稱，與上述步驟中所列的名稱不同。
...
## <a name="task-2-import-data"></a>工作 2：匯入資料  

1. 開啟 **[資料匯入]** 解決方案。

2. 檢查 **[匯入資料]** 流程的 **[狀態]** 。

3. 若 [狀態] 為 [關閉]，請選取 [匯入資料] 旁的 [...]，然後選取 [開啟]。

   > **重要事項：** 如果收到錯誤訊息，請確認您建立的資料表和資料行與上述指示相符。
