---
lab:
  title: 實驗室 3：如何建置模型導向應用程式
  module: 'Module 3: Describe how to build applications with Microsoft Power Apps'
---

# 實驗室 3：如何建置模型導向應用程式

**WWL 租用戶 - 使用條款**如果您針對講師導向訓練交付的一部分取得租用戶，請注意，租用戶的功能是為了在講師導向訓練中支援實際操作實驗室。 租用戶不應共用，或用於實際操作實驗室以外的用途。 本課程中使用的租用戶是試用租用戶，在課程結束後無法使用或存取租用戶，且不符合延伸模組的資格。 租用戶不得轉換成付費訂用帳戶。 此課程中提供的租用戶仍是 Microsoft Corporation 的財產，我們保留隨時取得存取權和重新持有的權利。 

## 案例

Bellows College 是一所教育組織，有多個校園和多項課程。 許多 Bellow College 的講師和管理員需要參加活動，並購買項目。 從歷史上來看，追蹤這些費用是一大挑戰。

校園管理希望為員工提供數位方式來報告費用，藉此將費用報告系統現代化。

在這整個課程中，您將建置應用程式及執行自動化，讓 Bellows College 員工能夠管理費用。

## 高階實驗室步驟

在建立模型驅動應用程式時，您將完成下列作業：

- 建立名為 Employee Expense Management 的新模型驅動應用程式。

- 編輯應用程式導覽以參考必要的數據表。

- 自訂應用程式所需數據表的表單和檢視。

我們將使用下列元件：

- **檢視：** 檢視可讓使用者在表單資料表中顯示現有的資料。

- **表單**：這是使用者在資料表中建立/更新新資料列的位置。

這兩者都會整合至模型驅動應用程式，以提供更好的使用者體驗。

### 必要條件

- 完成**單元 1 實驗室 0 - 驗證實驗室環境**

**開始前要考慮的事項**

- 我們應該進行哪些變更來改善使用者體驗？

- 我們應該根據我們建置的資料模型，在模型驅動應用程式中納入哪些項目？

- 模型驅動應用程式的網站地圖上可以進行哪些自訂？

## 練習 1：自訂檢視和表單

**目標：** 此練習中，您將自訂將在模型驅動應用程式中使用的自訂已建立資料表的檢視和表單。

### 任務 #1：編輯費用報表表表體

1. 若您尚未登入，請登入 https://make.powerapps.com

1. **如果尚未選取，請選取右上方的 Dev One** 環境。

1. 使用左側導覽，選取 [資料表]****，然後開啟 [費用報告]**** 資料表。

    >如果您沒看到 [費用報告] 資料表，請確保您處在正確的環境中 (步驟 2)。

1. 在 [資料體驗]**** 區段下方選取 [表單]****，然後按一下以開啟具有 [主要]**** 表單類型的 [資訊]**** 表單。 (**重要事項：** 請務必選取表單類型為 [主要]**** 的表單。)

    >**重要事項：** 由於所有表單的名稱皆預設為資訊，因此請務必確認您選取的表單具有 [主要] **** 表單類型，而不是其他類型。 依預設，表單具有兩個欄位：[名稱] 和 [擁有者]。

1. 在 [屬性] 面板的畫面**右側，選取 [**顯示名稱**] 字段，並將它變更為 `Report Information`。**

1. 在左側瀏覽窗格中選取 [資料表資料行]****，然後將資料行拖曳至表單或按一下資料行名稱，以在 [擁有者]**** 欄位的下方新增下列欄位：

    - **說明**

    - **報告目的**

    - **報告到期日**

    - **報告總金額**

1. 拖曳 [狀態原因]**** 資料行，然後將其放在表單標頭中。

    >標頭是表單的右上方區域。 您可能需要摺疊畫面右側的 [屬性] 面板，才能看到表單上的欄位。

1. 選取 [擁有者]**** 欄位。 在 [屬性] 面板中，將 [ **卷標** ] 變更為 `Requestor`。

1. 選取右上方的 [儲存並發佈]**** 按鈕，並等候儲存和發佈完成。

1. 如果編輯檢視在新的瀏覽器索引標籤或視窗中開啟，請予以關閉。 否則，選取畫面左上方的 [🡠 上一步]****。 您現在應該回到 [費用報告]**** 資料表表單。

1. 使用左上方的階層連結 ([資料表]**** > [費用報告]**** > [表單]****)。 選取 [費用報告]**** 以返回 [費用報告]**** 資料表屬性畫面。

## 任務 #2：編輯使用中費用報表檢視

在此工作中，我們將修改預設的 [作用中費用報告] 檢視，並為今天到期的費用報告建立新檢視。

1. 在 [資料體驗]**** 區段下方，選取 [檢視]****，然後開啟 [作用中費用報告]**** 檢視。

1. 按一下或拖放欄位，將下欄欄位新增至檢視：

    - **報告目的**

    - **報告到期日**

    - **報告總金額**

1. 選取 [建立於]**** 資料行上的下拉式功能表，然後選取 [移除]****。 [建立於]**** 欄位現在會從檢視中移除。

1. 將個別資料行寬度調整為適合資料的大小。

1. 在 [排序依據... ] **** 下 選取 X 以移除報表名稱 **，然後改為選取 [**報表總金額**]。**

1. 選取 [ **報表總金額]** ，將排序順序變更為 **[較大] 為 [較小**]。

1. 選取右上方的 [儲存並發佈]**** 按鈕，並等候發佈完成。

### 工作 #3：為今天到期的報表建立新檢視

現在，我們將複製檢視，為今天到期的報告建立新檢視。

>    **重要事項：** 請確定您未關閉 [作用中費用報告] 檢視，因為我們將會利用其來建立 [今天到期的報告] 檢視。

1. 選取**另存新檔**。

1. 將 [名稱]**** 變更為 `Expense Reports Due Today`，然後選取 [儲存]****。

1. 在 [屬性] 面板選取 [編輯篩選條件]****。

1. 選取 [+ 新增]****，選取 [新增資料列]****。

1. 選取 [報告到期日]**** 作為欄位，然後將 [等於]**** 變更為 [今天]****，作為下拉式清單中的條件。

1. 選取 **…** [狀態]**** 列上的 [更多指令]****，然後按一下 [刪除]**** 以刪除該篩選條件。

1. 選取 [確定] **** 以儲存條件。 檢視現在會進行篩選，以僅顯示 [報告到期日]**** 為今天的記錄。

1. 將 [報銷金額]**** 欄位新增至檢視。

1. 選取右上方的 [儲存並發佈]**** 按鈕，並等候發佈完成。

## 練習 2：建立模型驅動應用程式

**目標：** 在此練習中，您將建立模型驅動應用程式、自訂網站地圖，以及測試應用程式。

為了簡化和節省時間起見，我們將不會在此實驗室中處理所有 [費用報告] 資料行。

### 工作 #1：建立應用程式

1. 若您尚未登入，請登入 https://make.powerapps.com

1. 選取右上方的 [Dev One]**** 環境 (若尚未選取)。

1. 選取 **左側導覽上的 [解決方案** ]。

1. 開啟 **[費用管理解決方案** ]。

1. 選取 **[+新增**]，選取 **[應用程式** ]，然後選取 [ **模型驅動應用程式**]。

1. 針對 [名稱]**** 輸入 `Employee Expense Management`，然後選取 [建立]****。

1. 在新的模型驅動應用程式載入之後，選取 [ **+ 新增頁面** ] 按鈕。

1. 在 [ **新增] 頁面** 畫面上，選擇 **[Dataverse 數據表**]。

1. 選取下列資料表：

    - 費用報告

    - 連絡人

1. 在您擁有這兩個資料表之後，請選取 [新增]****。

1. 使用畫面左側的導覽圖示，選取 [導覽]****。

1. 在瀏覽窗格中，選取顯示瀏覽的 [新群組]****。 您可能需要展開左側的功能表。

1. 在畫面右側的 **[顯示選項** ] 區段中，將 **Title** 屬性變更為 `Reports`。

1. 選取 [儲存]****，然後等待變更儲存完畢。

1. [儲存]**** 完成後，請選取 [發佈]**** 按鈕以發佈變更。 等候發佈完成。

## 工作 #2：測試應用程式

**啟動應用程式**

1. 選取 [播放]**** 按鈕，模型驅動的應用程式將會載入新索引標籤。

**建立新的連絡人**

1. 應用程式應開啟至 [我的現行連絡人]**** 檢視。 如果沒有，請選取左側導覽的 [連絡人]****。

1. 從命令列選取 [+ 新的]****。

1. 輸入 [名字 **] 作為 `John` ，並將 **[姓氏]** 輸入**為 `Doe`。

1. 以 [電子郵件]**** 的形式提供您的個人電子郵件。 這會在未來的實驗室中使用，您將在其中收到電子郵件。

1. 選取**儲存&amp;關閉**。

1. 您現在應該會在 [我的現行連絡人]**** 檢視中看到已建立的連絡人。

**建立新的費用報告**

1. 從左側導覽 (也稱為網站地圖) 中選取 [費用報告]****。

1. 選取 **+ 新增**。

1. 如下所示輸入至欄位：

    - **報表名稱**： `New Test Report`

    - **報告用途**：選取 [會議]****

    - **報告到期日**：選取 [今日日期]

1. 選取**儲存後關閉**。 這將會建立新的測試報告，而且您應該能夠在 [作用中費用報告]**** 檢視上看到該報告。

1. 使用 [作用中費用報告]**** 旁邊的下拉式清單，將檢視變更為 [今天到期的費用報告]****。 

1. 您可以新增更多測試記錄。

執行中的模型驅動應用程式看起來應該大致如下：

![刪建立模型驅動應用程式的螢幕擷取畫面。](media/lab-3-create-a-model-app-01.png)

恭喜！ 您已建立並設定第一個模型驅動的應用程式。
