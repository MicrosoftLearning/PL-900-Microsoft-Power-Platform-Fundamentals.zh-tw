...
3.  使用左側導覽，選取 [ **資料表**]，然後開啟您的 **Visit** 資料表。

    如果您沒看到 [造訪情形] 資料表，請確保您處在正確的環境中 (步驟 2)。

4.  在 [**資料體驗] 區**段底下，選取 [**表單**]，然後**開啟 [主要**表單類型 **] 的資訊**表單。  (**重要事項：** 請務必選取表單類型為 **Main**.)  

    **重要事項：** 由於所有表單的名稱皆預設為「資訊」，因此請務必確認您選取的表單具有 [主要] 表單類型，而不是其他類型。 依預設，表單具有兩個欄位：[名稱] 和 [擁有者]。

5.  在 [ **屬性** ] 面板的畫面右側，選取 [ **顯示名稱** ] 欄位，並將它變更為 `Main Information`

6.  從左側流覽窗格中選取 **[資料表** 資料行]，並將下欄欄位新增至 **[擁有者** ] 欄位下方，方法是將資料行拖曳至表單，或直接按一下資料行名稱：
...
10. 選取右上方的 [ **儲存併發布** ] 按鈕，並等候儲存併發布完成。

11. 如果在新的瀏覽器索引標籤或視窗中開啟 [編輯] 檢視，請關閉它。 否則，請選取 🡠 畫面左上方的 **[返回** ]。 您現在應該回到 **Visit** 資料表表單。

12. 使用左上角的階層連結， (**資料表**  >  **流覽**  >  **表單**) 。 選取 **[流覽** ] 以返回 **[流覽** 資料表屬性] 畫面。 
...
1.  在 [ **資料體驗] 區** 段下，選取 [ **檢視** ]，然後開啟您的 **作用中流覽檢** 視。
...
4.  視需要調整個別資料行寬度的大小，以符合資料。

5.  在 **[排序依據... ]** 下 選取 X 以移除 **[名稱** ]，然後改為選取 [ **已排程的開始**]。

6.  選取 **[已排程的開始**] ，將排序次序變更**為 [較舊]。**
...
5.  選取 **[已排程的開始為]** 欄位，然後將 **[等於****今天**] 變更為下拉式清單中的條件。

6.  選取 **...** [**狀態**] 資料列上的**更多命令**，然後選取 [**刪除**] 以刪除該篩選準則。

7.  選取 **[確定** ] 以儲存條件。 檢視現在會篩選為只顯示 **排程開始日期** 為今天所在的記錄。
...
4.  建立模型導向應用程式：
    ...
    1. 在 [**建立您的應用程式**] 畫面的 [**開始]** 區段中，選取 **[空白應用程式**]。
