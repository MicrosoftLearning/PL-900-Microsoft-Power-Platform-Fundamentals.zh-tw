...
- 完成 **課程模組 0 實驗室 - 驗證實驗室環境**

- 完成 **課程模組 2 實驗室 - 資料建模**

- 完成 **課程模組 3 實驗室 - 如何建置畫布應用程式**
...
4. 在 [Data Experiences] (資料體驗) 區段下方選取 [表單]，然後按一下以開啟具有 [主要] 表單類型的資訊表單。

**重要事項：** 由於所有表單的名稱皆預設為「資訊」，因此請務必確認您選取的表單具有 [主要] 表單類型，而不是其他類型。 依預設，表單具有兩個欄位：[名稱] (主要欄位) 和 [擁有者]。
...
12. 如果編輯檢視在新的瀏覽器索引標籤或視窗中開啟，請予以關閉。 否則，按一下畫面左上方的 [上一步]。 您現在應已返回 [造訪情形] 資料表 [表單] 索引標籤。
...
1. 在 [Data experiences] (資料體驗) 區段下方，選取 [檢視]，然後按一下以開啟 [現行造訪情形] 檢視。
...
3. 建立模型導向應用程式：
    ...
    0. 在 [首頁] 畫面的 [起始] 區段中，選取 [空白應用程式]。

    1. 在 [根據 Dataverse 的空白應用程式] 底下，選取 [建立]。

    2. 針對 [名稱] 輸入 **Bellows 校園管理**，然後選取 [建立]。
...
2. 建立新的「連絡人」
    ...
    2. 在 [名字] 中輸入 John，[姓氏] 中則輸入 Doe。

    3. 提供您的個人電子郵件做為 **[電子郵件]** 。 這會在未來的實驗室中使用，您將在其中收到電子郵件。

    4. 按一下 [儲存並關閉]。

    5. 您現在應該會在 **[現行連絡人]** 檢視中看到已建立的連絡人。
...
3. 建立新的「造訪情形」
    ...
    2. 依照下列內容輸入欄位
        ...
        2. **預定開始時間**：選取明天的日期和下午 2:00 做為開始時間

        3. **預定結束時間**：選取明天的日期和下午 3:30 做為結束時間
