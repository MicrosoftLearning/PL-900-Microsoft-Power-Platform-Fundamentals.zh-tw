...
4. 在 [資料體驗] 區段下方選取 [表單]，然後按一下以開啟具有 [主要] 表單類型的資訊表單。

**重要事項：** 由於所有表單的名稱皆預設為「資訊」，因此請務必確認您選取的表單具有 [主要] 表單類型，而不是其他類型。 依預設，表單具有兩個欄位：[名稱] 和 [擁有者]。

5. 在畫面右側的 [屬性] 面板上，選取 [顯示名稱] 欄位，並將其變更為**主要資訊**。
...
9. 選取 **[擁有者]** 欄位。 在 [屬性] 面板中，將 [標籤] 變更為 [負責人]

10. 按一下右上方的 **[儲存]** ，然後等待儲存完成。

11. 按一下右上方的 **[發佈]** ，然後等待發佈完成。

12. 如果編輯檢視在新的瀏覽器索引標籤或視窗中開啟，請予以關閉。 否則，按一下畫面左上方的 [上一步]。 您現在應已返回 [造訪情形] 資料表 [表單]。

13. 使用左上方的階層連結 ([資料表] > [造訪情形] > [表單])。 選取 [造訪情形]，以返回 [造訪情形] 資料表主畫面。
...
## <a name="exercise-2-create-model-driven-app"></a>練習 2：建立模型驅動應用程式

**目標：** 在此練習中，您將建立模型驅動應用程式、自訂網站地圖，以及測試應用程式。

為了簡單和節省時間起見，有部分 [造訪] 資料行不會在此實驗中處理。

### <a name="task-1-create-app"></a>工作 \#1：建立應用程式
...
3. 建立模型導向應用程式：
    ...
    1. 在 [根據 Dataverse 的空白應用程式] 底下，選取 [建立]。

    1. 針對 [名稱] 輸入 **Bellows 校園管理**，然後選取 [建立]。
...
9. 在瀏覽窗格中，選取顯示瀏覽列的 [群組 1]。 您可能需要展開左側的功能表。

10. 在畫面右側的 [顯示選項] 區段中，將 [標題] 屬性變更為 [安全性]。

### <a name="task-2-edit-your-app"></a>工作 2：編輯您的應用程式

既然我們已將所有必要的元件新增至模型驅動應用程式，我們現在會組織項目。

1. 在瀏覽窗格的安全性群組底下，選取 [SubArea1]。

2. 選取**省略符號**，並在顯示的功能表中選取 **Remove SubArea1**。

3. 使用畫面左側的導覽，選取 [頁面]。

4. 在 [頁面] 窗格上找到並展開 [造訪情形]。
...
14. [儲存] 完成後，請選取 [發佈] 按鈕以發佈變更。

### <a name="task-3-test-application"></a>工作 #3：測試應用程式
...
2. 建立新的「連絡人」
    ...
    1. 在 [名字] 中輸入 John，[姓氏] 中則輸入 Doe。

    1. 提供您的個人電子郵件做為 **[電子郵件]** 。 這會在未來的實驗室中使用，您將在其中收到電子郵件。
...
3. 建立新的「造訪情形」
    ...
    1. 依照下列內容輸入欄位
        ...
        1. **預定開始時間**：選取明天的日期和下午 2:00 做為開始時間

        1. **預定結束時間**：選取明天的日期和下午 3:30 做為結束時間

- 按一下 [儲存並關閉]****。 這會建立 Visit，而且應會顯示在 [現行瀏覽] 檢視中

- 使用 [Active Visits] 旁的下拉式清單將檢視變更為 [Today’s Visits]。 您應該不會再於此檢視中看到新的造訪情形，因為其預定時間為明天。
