...
ms.locfileid: "137898856"
---
# <a name="module-3-get-started-with-power-apps"></a>模組 3：開始使用 Power Apps
## <a name="lab-2-how-to-build-a-canvas-app-part-2"></a>實驗室 2：如何建置畫布應用程式 (第 2 部分)
...
# <a name="exercise-1-create-security-canvas-app"></a>練習 \#1：建立安全性畫布應用程式
...
2.  建立新的畫布應用程式
    ...
    -   按一下 [新增]，然後選取 [應用程式] \| [畫布應用程式]。

    -   在畫布應用程式空白視窗中的 [應用程式名稱] 欄位中，輸入 **[您的姓氏] Campus Security** 。
...
3.  儲存畫布應用程式
    ...
    - 確定 [名稱] 欄位中是 **[您的姓氏] Campus Security**，然後按一下 [儲存]。
...
## <a name="task-2-display-visitor-information"></a>工作 \#2：顯示訪客資訊
...
2.  編輯文字輸入物件
    ...
    -   選取 **Hint Text** 屬性，然後輸入 `"Enter visitor code"` 作為值 (含雙引號)
    
    -   在樹狀檢視 (TextInput1) 中，按一下控制項名稱旁的 [...]，然後選取 [重新命名]，將名稱變更為 `textCode`
...
## <a name="task-3-add-check-in-and-check-out-buttons"></a>工作 \#3：新增「簽入」和「簽出」按鈕
...
2. 新增 [簽入] 按鈕
   ...
   * 在 [屬性] 窗格中，將按鈕的 **Text** 屬性，變更為 "`Check In`" (您可以在現有的引號中鍵入)
   
   * 在樹狀檢視中 (Button1)，按一下按鈕名稱旁的 [...]，然後選取 [重新命名]，將名稱變更為 `CheckInButton`

3. 新增 [簽出] 按鈕   

   * 按一下 [插入] 索引標籤上的 **[按鈕]** 以插入其他按鈕
   
   * 在 [屬性] 窗格中，將按鈕的 **Text** 屬性，變更為 "`Check Out`" (您可以在現有的引號中鍵入)
   
   * 將按鈕重新命名為 `CheckOutButton`
   
   * 將按鈕置於搜尋方塊下方，然後 **[簽入]** 位於 **[簽出]** 上方 
   
## <a name="task-4-enable-and-disable-buttons-depending-on-visit-data"></a>工作 4\#：根據造訪情形資料啟用和停用按鈕
...
2. 在函數列中輸入以下運算式：
   ...
   * **!IsBlank(Visit)** - 找到瀏覽記錄
   * **&&** - 邏輯運算子 AND
   * **Visit.Status = 'Status (Visits)'.Active** 記錄狀態為 *現行*
   * **IsBlank(Visit.'Actual Start')** - [Active Start] 欄位中沒有任何資料
   * **DisplayMode.Edit, DisplayMode.Disabled** - 當符合上述條件時，此按鈕會變成可以編輯。 如果不符合，則按鈕將維持停用狀態。
...
## <a name="task-5-complete-check-in-and-check-out-process"></a>工作 \#5：完成「簽入」和「簽出」程序
...
2. 將 [進階] 索引標籤上的 **OnSelect** 屬性設為下列運算式。
   ...
   * **Patch(Visits, Visit, {'Actual Start':Now()});** . *Patch* 方法會更新 **[造訪情形]** 資料表，也就是由 **Visit** 變數識別的資料列 (即目前的造訪情形)。 此運算式會將 *[Actual Start]* 資料行，設定為目前的日期和時間 ( *(Now()* 方法)。
...
## <a name="task-6-add-visual-indicators"></a>工作 \#6：新增視覺效果指標
...
## <a name="task-7-publish-the-app"></a>工作 #7：發佈應用程式
