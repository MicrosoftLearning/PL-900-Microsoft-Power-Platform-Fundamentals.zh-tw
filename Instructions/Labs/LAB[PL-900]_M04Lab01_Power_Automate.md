...
## <a name="task-1-create-flow"></a>工作 \#1：建立流程
...
2.  按一下 [新增]，再依序選取 [自動化]、[雲端流程] 和 [已自動化]。 此動作會在新視窗中開啟 Power Automate 流程編輯器。

3. 在 [Choose your flow's trigger] (選擇您流程的觸發程序) 中，搜尋 **Microsoft Dataverse**。

4. 選取觸發程序 [When a row is added, modified or deleted] (當有資料列新增、修改或刪除時)，然後按一下 [建立]。
   ...
   * 按一下觸發程序步驟上的省略符號 ( [...])，然後按一下 [重新命名]。 將此觸發程序重新命名為 **"When a visit is added"** 。 這是個很好的練習，讓您和其他流程編輯者均可瞭解步驟的目的，而不必深入探討細節。

5. 選取 [新增步驟]。 此為擷取訪客資訊 (包括電子郵件地址) 的必要步驟。
...
7. 選取 **[依識別碼取得資料列]** 動作。 
   ...
   * 按一下此動作上的省略符號 ([...])，然後按一下 [重新命名]。 將此動作重新命名為 **「取得訪客」** 。 這是個很好的練習，讓您和其他流程編輯者均可瞭解步驟的目的，而不必深入探討細節。

8. 按一下 **[新增步驟]** 。 此步驟可建立電子郵件並將其傳送給訪客。

9. 搜尋 *郵件*，再依序選取 [Office 365 Outlook]連接器及 [傳送電子郵件 (V2)] 動作。
...
10.  選取最上方 [未命名] 流程名稱，將其重新命名為 `Visit notification`
...
14.  在命令列上按一下 **[關閉]** 以關閉流程。 您必須按省略符號 ( [...])，才會顯示此選項。

15.  關閉此視窗。

# <a name="exercise-2-create-security-sweep-flow"></a>練習 2：建立「安全掃描」流程

**目標：** 在此練習中，您將建立可實作需求的 Power Automate 流程。 每 15 分鐘就必須執行安全掃描，且如果有任何訪客的停留時間超過其預定時間，則應該通知保全單位。

## <a name="task-1-create-flow-to-retrieve-records"></a>工作 #1：建立流程以擷取記錄
...
2. 按一下 [新增]，再依序選取 [自動化]、[雲端流程] 和 [已排程]。 此動作會在新視窗中開啟 Power Automate 流程編輯器。

3. 將該流程設定為每 **15** 分鐘重複一次。
...
5. 按一下 **[新增步驟]** 。 選取 *[目前]* ，然後選取 **[Microsoft Dataverse]** 連接器。 選取 **[列出資料列]** 動作。
   ...
   * 按一下此動作上的省略符號 ([...])，然後按一下 [重新命名]。 將此動作重新命名為 **「列出在 15 分鐘前已結束的現行造訪情形」** 這是個很好的練習，讓您和其他流程編輯者均可瞭解步驟的目的，而不必深入探討細節。
...
8.  擷取「大樓」資料以取得相關記錄
    ...
    * 按一下 [取得記錄] 旁的 [...]，然後選取 [重新命名]。 輸入 **取得大樓** 做為步驟名稱
...
9.  擷取「訪客」資料以取得相關記錄
    ...
    * 按一下 [取得記錄] 旁的 [...]，然後選取 [重新命名]。 輸入 **取得訪客** 做為步驟名稱
    
10.  傳送電子郵件通知

     * 在 [套用至每個項目] 迴圈中按一下 **[新增動作]** 。 從 **Office 365 Outlook** 連線，新增 [傳送電子郵件 (V2)] 動作。
...
13.  在 **[本文]** 欄位中輸入下列內容。 **[名稱]** 為 **取得大樓** 步驟中的動態內容。 您必須捲動到清單底部。
